		...
		if(FD_ISSET(s, &read_fds)) {
			...
			if(client_fd != -1)  {
				...
				if(num_chat!=1)
				    ...
				    send(client_fd, start0, strlen(start0), 0);
				printf("%d번째 사용자 추가.\n",num_chat);
			}
		}
					...
					if (strstr(rline,"Exit/") != NULL) {
						if(strcmp(user[i].name,user[0].name)!=0)
						{
							...
							if(strcmp(e_name,user[j].name)==0){puts("ok");
							    send(user[j].client_s,ExitMessage2,strlen(ExitMessage2),0);
							    printf("방장 %s에 의한 %s 강퇴\n",user[0].name,e_name);
							    shutdown(user[j].client_s, 2);
							    if(i != num_chat-1)
								user[j].client_s = user[num_chat-1].client_s;
							    num_chat--;
							    strcpy(rline,e_name);
